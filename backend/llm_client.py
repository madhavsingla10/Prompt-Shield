...
class LLMClient:
    """
    Unified asynchronous LLM client supporting Featherless.AI (Open-weights models like Llama 3, Mistral),
    Google Gemini API, and OpenAI API.
    """
    ...
    def detect_provider(self, model: str) -> str:
        ...
        if "gemini" in model_lower:
            ...
            # If featherless key is set, use featherless; otherwise fallback if available
    ...
    async def generate(
        ...
        """
        Unified generation method returning structured result with text, latency, tool_calls, and metadata.
        """
        ...
        try:
            if provider == "gemini":
                ...
                # No provider key configured - raise informative error
                raise ValueError("No LLM API keys configured. Please configure FEATHERLESS_API_KEY, GEMINI_API_KEY, or OPENAI_API_KEY.")
    ...
    async def _generate_gemini(
        ...
        # Ensure model ID is formatted for Gemini SDK
        gemini_model = model
        if not gemini_model.startswith("gemini-"):
            gemini_model = "gemini-2.5-flash"

        # Execute in async thread pool since python genai client calls may be blocking or async
        def _sync_call():
            config: Dict[str, Any] = {
                "temperature": temperature,
